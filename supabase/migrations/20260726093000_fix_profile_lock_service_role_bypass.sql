/*
# Fix service-role/admin bypass detection in profile privilege lock trigger

The previous trigger relied on request.jwt.claim.role in a SECURITY DEFINER
function. In some runtime paths this claim can be unavailable, causing service
operations to be blocked.

This migration replaces the trigger function with an invoker-context function
that resolves role from multiple sources and preserves strict protected-field
locking for non-admin users.
*/
